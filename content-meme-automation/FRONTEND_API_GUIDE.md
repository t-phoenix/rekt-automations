...
* **Payload Config (JSON):**
  * `user_text` *(string, optional)*: If provided, dynamically overrides the trend pipeline and immediately generates a meme based on this exact text input. PERFECT for passing in manual user creations from the frontend.
  * `platform` *(string, optional)*: When using `user_text`, explicitly set the mock platform style (e.g. `twitter`, `linkedin`).
  * `theme` *(string, optional)*: A specific topic prompt (e.g. "when the dev rugs").
  * `tone` *(string, optional)*: Humor style.
  * `template_preference` *(string, optional)*: Offer users a dropdown of known template categories (e.g., "reaction_memes"). Forces the LLM to skip its own discovery and use this template aesthetic.
...
2. **Hit the corresponding `POST /workflows/...`** endpoint with the JSON payload.
   ...
   const res = await fetch("http://localhost:8000/api/workflows/meme-generation", {
     ...
     body: JSON.stringify({
       user_text: "Just bought the top again, someone take my phone away.",
       platform: "twitter",
